...
function IconGrip(props: React.SVGProps<SVGSVGElement>) {
  return (
...
export default function AdminDashboard() {
  ...
  // New Visual Drag-and-Drop Reordering State
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (failedItems.length > 0) {
      ...
      // Delay to allow Cloudinary propagation before refreshing data
      ...
      // Delay to allow Cloudinary propagation before switching views
      await new Promise((resolve) => setTimeout(resolve, 1500));
      await loadData();
  ...
  const startReorderMode = () => {
    setLocalOrderedMedia([...filteredMedia]);
    setReorderMode(true);
  };
  ...
  const handleDragEnter = (index: number) => {
    ...
    const items = [...localOrderedMedia];
    const draggedItemContent = items.splice(dragItemRef.current, 1)[0];
    items.splice(dragOverItemRef.current, 0, draggedItemContent);

    dragItemRef.current = dragOverItemRef.current;
    setLocalOrderedMedia(items);
  };
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-[var(--plum)]/30 border border-[var(--rose)]/30 p-3.5 px-4 rounded-xl">
                      <div className="flex items-center gap-2 text-[var(--rose)]">
                        <IconGrip className="w-4 h-4 shrink-0" />
                        <span className="text-sm font-medium">Drag and drop the photos to arrange their order</span>
                      </div>
                      <div className="flex gap-2 shrink-0">
                        <GhostButton onClick={() => setReorderMode(false)} disabled={isReordering}>Cancel</GhostButton>
                        <PrimaryButton onClick={saveNewOrder} loading={isReordering}>Save Order</PrimaryButton>
                      </div>
                    </div>
                  </motion.div>
                )}
                    ...
                    return (
                      ...
                      >
                        <div
                          ...
                          className={`absolute inset-0 overflow-hidden bg-black/40 ${!reorderMode ? 'cursor-pointer' : ''}`}
                        >
                          {item.type === "VIDEO" ? (
                            <video
                              src={item.url}
            ...
            <AnimatePresence>
              {selectMode && selectedMediaIds.size > 0 && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 50 }}
                  className="fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-50 bg-[var(--paper)] border border-[var(--line)] shadow-2xl p-2.5 sm:p-3 px-4 sm:px-6 rounded-2xl flex items-center justify-between gap-4 w-[90vw] sm:w-auto sm:min-w-[450px] backdrop-blur-xl"
                >
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-[var(--rose)] text-white flex items-center justify-center text-xs font-bold shadow-inner">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      title="Move to Album"
                      className="flex items-center gap-2 py-2 px-2 sm:px-3.5 rounded-xl bg-[var(--bg)] border border-[var(--line)] hover:border-[var(--rose)]/50 text-[var(--cream)] transition-all cursor-pointer"
                    >
                      <IconFolder className="w-4 h-4 text-[var(--gold-soft)]" />
                      <span className="hidden sm:block text-xs font-medium">Move</span>
                    </button>

                    <button
                      onClick={() => setShowBulkDeleteModal(true)}
                      title="Delete Selected"
                      className="flex items-center gap-2 py-2 px-2 sm:px-3.5 rounded-xl bg-rose-500/10 border border-rose-500/20 hover:bg-rose-500/20 text-rose-400 transition-all cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl text-[var(--cream)]/50 hover:text-[var(--cream)] hover:bg-[var(--bg)] transition-colors cursor-pointer"
                    >
